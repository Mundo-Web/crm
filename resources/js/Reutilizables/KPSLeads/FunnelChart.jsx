...
export const FunnelChart = ({ data = {}, extraData = [], title = "Funnel de Conversión - Landing", showRates = true }) => {
  ...
  const funnelData = [
    ...(data.impressions !== undefined ? [{ stage: 'Impresiones', count: data.impressions, color: '#3B82F6' }] : []),
    ...extraData,
    ...(data.contacted !== undefined ? [{ stage: 'Contacto realizado', count: data.contacted, color: '#F59E0B' }] : []),
    ...(data.salesClosed !== undefined ? [{ stage: 'Venta cerrada', count: data.salesClosed, color: '#10B981' }] : [])
  ];
