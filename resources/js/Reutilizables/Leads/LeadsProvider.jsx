...
export const LeadsProvider = ({
    ...
    const [selectedUsersId, setSelectedUsersId] = useCrossTabSelectedUsers(
        LaravelSession.business_id,
        [LaravelSession.service_user.id],
    );
    const [defaultView, setDefaultView] = useState(
        Local.get("default-view") ?? "kanban",
    );
    ...
    const getLeads = () => {
        statuses
            ...
            filter.push(['created_at', '<', nextMonth.toISOString().slice(0, 10)]); */
                ...
                setLeadsCount((old) => ({
                    ...old,
                    [statusId]: response.totalCount,
                }));
                setLeads((prevLeads) => {
                    const newLeads = [...prevLeads];
                    response.data.forEach((newLead) => {
                        const existingLeadIndex = newLeads.findIndex(
    ...
    const refreshLeads = async () => {
        const dateWithOffset = new Date(
            lastLoadedDate.getTime() - 5 * 60 * 60 * 1000,
        );
        ...
        if (selectedUsersId.length > 0) {
            filter.push(
                ArrayJoin(
                    selectedUsersId.map((id) => ["assigned_to", "=", id]),
                    "or",
                ),
            );
        }
