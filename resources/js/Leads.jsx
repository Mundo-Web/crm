...
import {
    LeadsContext,
    LeadsProvider,
} from "./Reutilizables/Leads/LeadsProvider.jsx";
...
const Leads = (properties) => {
    const {
        ...
        defaultLeadStatus,
        convertedLeadStatus,
        manageStatuses: manageStatusesFromDB,
        noteTypes,
        products = [],
        processes = [],
        projectTypes = [],
    ...
    const onDeleteNote = async (noteId) => {
        ...
        const newNotes = structuredClone(notes).filter(
            ({ id }) => id != noteId,
        );
        setNotes(newNotes);
    };
    ...
    const onClientStatusClicked = async (lead, status) => {
        let leadId = typeof lead === "object" ? lead.id : lead;
        let leadData = typeof lead === "object" ? lead : leadLoaded;
        ...
        if (defaultClientStatus && status == defaultClientStatus) {
            const wasModalOpen = $(modalRef.current).hasClass("show");
            if (wasModalOpen) {
                $(modalRef.current).modal("hide");
            }

            const formValues = await confirmLeadConversion(leadData);

            if (!formValues) {
                if (wasModalOpen) {
                    setLeadLoaded(leadData);
                    history.pushState(null, null, `/leads/${leadId}`);
                    setNotes([]);
                    $(modalRef.current).modal("show");
                }
                return;
        ...
        await leadsRest.leadStatus({ lead: leadId, status });

        if (leadLoaded && leadLoaded.id == leadId) {
    ...
    const onManageStatusChange = async (lead, status) => {
        ...
        if (
            ...
            result = await leadsRest.leadStatus({
                lead: lead.id,
        ...
        result = await leadsRest.manageStatus({
            lead: lead.id,
            status: status.id,
        });
    ...
    const onPhoneChange = async (saving = false) => {
        ...
        if (result?.totalCount > 0) {
            ...
            const { isConfirmed } = await Swal.fire({
                ...
                text: lead.creator
                    ? `${lead.creator.fullname} ha registrado este lead anteriormente con el nombre de ${lead.contact_name}`
                    : `Este numero de telefono ha sido registrado anteriormente con el nombre de ${lead.contact_name}`,
                icon: "warning",
    ...
    const onMakeLeadClient = async (data) => {
        onClientStatusClicked(data, defaultClientStatus);
    };
    ...
    const pendingTasks = [];
    notes?.forEach((note) =>
        pendingTasks.push(...note.tasks.filter((x) => x.status != "Realizado")),
    );
    ...
    const handleImport = (file) => {
        ...
        if (!validTypes.includes(file.type)) {
            Swal.fire(
                "Formato inválido",
                "Solo se permiten archivos .xlsx, .xls, .csv o .xml",
                "warning",
            );
            return;
        }
            ...
            if (
                ...
                rows.forEach((row, rIdx) => {
                    ...
                    cells.forEach((cell, cIdx) => {
                        worksheet.getCell(rIdx + 1, cIdx + 1).value =
                            cell.textContent;
                    });
                });
                ...
                try {
                    ...
                    Swal.fire(
                        ...
                        "No se pudo leer el archivo Excel. Asegúrate de que sea .xlsx o .csv",
                        "error",
                    );
                    return;
                }
    ...
    return (
                    ...
                    <div className="d-flex gap-1 justify-content-between align-items-center">
                        ...
                        <div className="d-flex gap-0">
                            <input
                                ...
                                type="radio"
                                name="view-as"
                                defaultChecked={defaultView == "table"}
                                onClick={() => onDefaultViewClicked("table")}
                            />
                            ...
                            <input
                                ...
                                type="radio"
                                name="view-as"
                                defaultChecked={defaultView == "kanban"}
                                onClick={() => onDefaultViewClicked("kanban")}
                            />
                        ...
                        <button
                            className="btn btn-sm btn-purple driver-js-btn-new-lead"
                            onClick={() => onOpenModal()}
                        >
        ...
        >
            {defaultView == "table" ? (
                <>
                    <LeadTable
                        gridRef={managedGridRef}
                        otherGridRef={gridRef}
                        rest={leadsRest}
                        can={can}
                                ...
                                >
                                    <button
                                        ...
                                        style={{
                                            color: "#ffffff",
                                            backgroundColor:
                                                leadLoaded?.status?.color ||
                                                "#6c757d",
                                        }}
                                        disabled
                                    >
                                        {leadLoaded?.status?.name ||
                                            "Sin estado"}{" "}
                    ...
                    <div className="col-lg-3 col-md-4 col-sm-6 col-xs-12">
                        <div className="card">
                            <div className="card-header bg-danger">
                                <h5 className="header-title my-0 text-white">
                                    Tareas
                                </h5>
                            </div>
                        ...
                        <div className="card">
                            ...
                            <div className="card-body">
                                <Dropdown
                                    className="d-block mx-auto btn btn-sm btn-white rounded-pill"
                                    title="Agregar"
                                    icon={{ icon: "fa fa-plus" }}
                                    tippy="Agregar producto al cliente"
                                >
                                ...
                                <div className="mt-2 d-flex flex-column gap-2">
                                    {console.log(
                                        "productos de leads del cliente leads",
                                        clientProducts,
                                    )}
            ...
            <Modal
                ...
                zIndex={1060}
                btnAcceptClass="driver-js-btn-save"
            >
            ...
            >
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td className="py-0 px-1">
                                {moment(new Date(mailLoaded?.date)).format(
                                    "LLL",
                                )}
                            </td>
                        </tr>
                ...
                <hr className="my-2" />
                <HtmlContent
                    html={mailLoaded?.bodyHtml}
                    style={{
                        minHeight: "360px",
                    }}
                />
                <hr className="mt-2 mb-0" />
                {mailLoaded?.attachments?.length > 0 && (
                    <div className="mt-2 d-flex flex-wrap gap-2">
                        {mailLoaded?.attachments?.map((file) => {
                            return (
                                <div
                                    className="d-flex gap-1 border p-1"
                                    style={{
                                        width: "180px",
                                    }}
                                >
                                    ...
                                    <div>
                                        ...
                                        <div className="d-flex gap-1 align-items-center justify-content-between">
                                            ...
                                            <div className="d-flex gap-1">
                                                <Tippy content="Abrir">
                                                    <a
                                                        href={`/api/gmail/attachment/${mailLoaded?.id}/${file.attachmentId}/${file.filename}`}
                                                        target="_blank"
                                                        className="btn btn-xs btn-white"
                                                        type="button"
                                                    >
                ...
                {mailLoaded?.sender?.includes(leadLoaded?.contact_email) && (
                    <div className="mt-2">
                        <button
                            className="btn btn-xs btn-white rounded-pill"
                            type="button"
                            onClick={() => {
            ...
            <OffCanvas
                offCanvasRef={messagesOffCanvasRef}
                dataLoaded={leadLoadedForMessages}
                setDataLoaded={setLeadLoadedForMessages}
                defaultMessages={defaultMessages}
                onOpenModal={onOpenModal}
                onLeadClicked={onLeadClicked}
                signs={signs}
