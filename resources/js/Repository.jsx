...
const Repository = ({ files: filesDB }) => {
  ...
  const handleFiles = async (files) => {
    ...
    for (const file of files) {
      ...
      try {
        const result = await repositoryRest.save(formData, false)
        if (result) {
  ...
  const onCopyLinkClicked = ({ name, file }) => {
    ...
    Clipboard.copy(download_link, () => {
      Notify.add({
        ...
        body: `El enlace al archivo ${name} ha sido copiado al portapapeles`,
        icon: '/assets/img/icon.svg'
      })
    })
  }
  ...
  return (
    <>
      <div
        className={`repository-container ${isDragging ? 'dragging' : ''}`}
      ...
      >
        ...
        <div className="repository-dropzone">
          {isDragging ? (
      ...
      <Modal modalRef={modalRef} title='Modificar nombre' onSubmit={onModalSubmit}>
